...
const displayName = "Blued" + "Code"
const slug = "blued" + "code"
const brandDirectory = "xcode/build/"
const excluded = ["docs/superpowers/specs/", ".test.", "/test/", "\\test\\"]
    ...
    files.stdout
      ...
      .map(async (file) => {
        ...
        const source = await Bun.file(target)
          .text()
          .catch(() => "")
        return source.includes(displayName) || source.includes(slug) ? file : undefined
      }),
  )
).filter((file): file is string => !!file)

if (violations.length)
  throw new Error(`Hardcoded product identity outside the explicit preset:\n${violations.join("\n")}`)
