...
const productLink = (value: string) => `${Brand.protocol}://${value}`
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink(productLink("open-project?directory=/tmp/demo"))).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink(productLink("other?directory=/tmp/demo"))).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink(productLink("open-project/%E0%A4%A%"))).not.toThrow()
    expect(parseDeepLink(productLink("open-project/%E0%A4%A%"))).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink(productLink("open-project"))).toBeUndefined()
    expect(parseDeepLink(productLink("open-project?directory="))).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      productLink("open-project?directory=/a"),
      productLink("other?directory=/b"),
      productLink("open-project?directory=/c"),
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink(productLink("new-session?directory=/tmp/demo"))).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink(productLink("new-session?directory=/tmp/demo&prompt=hello%20world"))).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink(productLink("new-session"))).toBeUndefined()
    expect(parseNewSessionDeepLink(productLink("new-session?directory="))).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      productLink("new-session?directory=/a"),
      productLink("open-project?directory=/b"),
      productLink("new-session?directory=/c&prompt=ship%20it"),
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCODE__: {
        deepLinks: [productLink("open-project?directory=/a")],
      },
    } as unknown as Window & { __OPENCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual([productLink("open-project?directory=/a")])
