...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      `${Brand.name} window terminated unexpectedly`,
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show(`${Brand.name} is not responding`, "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
