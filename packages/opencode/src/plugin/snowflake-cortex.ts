...
function authHeaders() {
  return {
    ...
    "User-Agent": `${Brand.slug}/${InstallationVersion}`,
  }
}
...
<html>
  <head><title>${Brand.name} - Snowflake Authorization Successful</title></head>
...
<html>
  <head><title>${Brand.name} - Snowflake Authorization Failed</title></head>
  <body style="font-family: system-ui; display:flex; align-items:center; justify-content:center; height:100vh; margin:0; background:#111; color:#eee;">
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            const prepareRequest = () => {
              ...
              headers.set("User-Agent", `${Brand.slug}/${InstallationVersion}`)
      ...
      methods: [
        {
          ...
          async authorize(inputs = {}) {
            ...
            return {
              url,
              instructions: `Complete Snowflake sign-in in your browser. ${Brand.name} will capture the OAuth callback and store the bearer token automatically.`,
