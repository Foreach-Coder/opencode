...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: `fetch and checkout a GitHub PR branch, then run ${Brand.name}`,
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          const importResult = yield* Effect.promise(() =>
            Process.text([Brand.cli, "import", sessionUrl], { nothrow: true }),
          )
    ...
    const code = yield* Effect.promise(
      () =>
        Process.spawn([Brand.cli, ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`${Brand.cli} exited with code ${code}`))
  }),
})
