...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro(`Add servers with: ${Brand.cli} mcp add`)
      return
    }
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: `e.g., ${Brand.cli} x @modelcontextprotocol/server-filesystem`,
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: `${Brand.slug}-debug`, version: InstallationVersion },
            },
            id: 1,
          }),
        })
