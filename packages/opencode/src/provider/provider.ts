  ...
  return {
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `${Brand.slug}/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `${Brand.slug}/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            `Set it via environment variable or run \`${Brand.cli} auth cloudflare-ai-gateway\`.`,
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `${Brand.slug}/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, ${Brand.cli} auth, or provider options.`,
            )
          },
        }
      }
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const dep = {
          auth: (id: string) => (Brand.enterprise ? Effect.succeed(undefined) : auth.get(id).pipe(Effect.orDie)),
          config: () => config.get(),
          env: () => (Brand.enterprise ? Effect.succeed({}) : env.all()),
          get: (key: string) => (Brand.enterprise ? Effect.succeed(undefined) : env.get(key)),
        }
        ...
        function isProviderAllowed(providerID: ProviderV2.ID): boolean {
          if (Brand.enterprise && !configured.has(providerID)) return false
        ...
        for (const hook of plugins) {
          ...
          if (Brand.enterprise && !configured.has(providerID)) continue
          if (disabled.has(providerID)) continue

          const provider = database[providerID]
          if (!provider) continue
          const pluginAuth = Brand.enterprise ? undefined : yield* auth.get(providerID).pipe(Effect.orDie)
